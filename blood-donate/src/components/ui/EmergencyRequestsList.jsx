// Map bloodTypeID to blood type name
...
const EmergencyRequestsList = ({ onSupport }) => {
  ...
  if (!requests.length) return <Alert variant="info">Không có yêu cầu khẩn cấp nào đang mở.</Alert>;
      ...
      {requests.map((req, idx) => (
        <Col md={6} lg={4} key={req.requestId || idx} className="mb-4">
          <Card border="danger" className="h-100">
            <Card.Body>
              <div className="mb-2"><Badge bg="danger">Khẩn cấp</Badge></div>
              <div><strong>Nhóm Máu:</strong> {
                req.bloodTypeName
                || BLOOD_TYPE_MAP[(req.bloodTypeRequired || '').toString().trim()]
                || req.bloodTypeRequired
                || (req.aboType && req.rhFactor ? `${req.aboType}${req.rhFactor}` : '')
              }</div>
              <div><strong>Số lượng:</strong> {req.quantityNeeded}ml</div>
              <div><strong>Mô tả:</strong> {
                req.description && /^Emergency request\. Available units:/i.test(req.description.trim())
                  ? req.description.replace(/^Emergency request\. Available units:[^\.]*\.(\s*)?/i, '').trim()
                  : req.description
              }</div>
              <Button 
