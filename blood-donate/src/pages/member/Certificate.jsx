...
const Certificate = () => {
  ...
  const loadCertificates = async () => {
    try {
      ...
      const mapped = (
        Array.isArray(apiCertificates) ? apiCertificates : []
      ).map((c) => ({
        id: c.certificateId || c.id,
        certificateNumber: c.certificateNumber,
        issueDate: c.issueDate,
        type:
          c.certificateType === "Blood Donation" ? "donation" : "achievement",
...
${
  certificate.type === "donation"
    ? `Ngày hiến máu: ${dayjs(certificate.bloodDonationDate).format(
        "DD/MM/YYYY"
      )}
...
Hiệu lực đến: ${
      certificate.validUntil === "permanent"
        ? "Vĩnh viễn"
        : dayjs(certificate.validUntil).format("DD/MM/YYYY")
    }`;
            ...
            <Row gutter={[16, 16]} justify="center">
              <Col span={12}>
                ...
                <Text strong style={{ color: "white", fontSize: "16px" }}>
                  {dayjs(selectedCertificate.bloodDonationDate).format(
                    "DD/MM/YYYY"
                  )}
                </Text>
              </Col>
        ...
        >
          <Row justify="space-between" align="middle">
            ...
            <Col>
              ...
              >
                Ngày cấp:{" "}
                {dayjs(selectedCertificate.issueDate).format("DD/MM/YYYY")}
              </Text>
            </Col>
          </Row>
        </div>
                      ...
                      {certificate.type === "donation" ? (
                        <div>
                          <Text>
                            ...
                            {dayjs(certificate.bloodDonationDate).format(
                              "DD/MM/YYYY"
                            )}
                          </Text>
