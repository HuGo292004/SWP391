...
const handleResponse = async (response) => {
  ...
  if (!response.ok) {
    ...
    switch (response.status) {
      ...
      case 400:
        throw new Error(
          data.message || data.Message || "Dữ liệu không hợp lệ."
        );
      default:
        throw new Error(
          data.message || data.Message || `Lỗi server: ${response.status}`
        );
    }
  }
...
export const getUsersByRole = async (role) => {
  try {
    ...
    // Try different role formats that the API might expect
    const roleFormats = [
      role, // Original format
      role.charAt(0).toUpperCase() + role.slice(1).toLowerCase(), // Title case
    ...
    for (const formattedRole of roleFormats) {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/User/Get-User-By-Role/${encodeURIComponent(
            formattedRole
          )}`,
          {
            method: "GET",
        ...
        debugLog(
          `API call attempt: ${API_BASE_URL}/api/User/Get-User-By-Role/${encodeURIComponent(
            formattedRole
          )}`
        );
        ...
        if (response.ok) {
          const data = await handleResponse(response);
          debugLog(
            `Users by role ${role} (format: ${formattedRole}) fetched successfully`,
            data
          );
          return data;
        } else if (response.status !== 404 && response.status !== 400) {
          // If it's not a "not found" or "bad request", throw the error
          await handleResponse(response);
        }
      } catch (formatError) {
        lastError = formatError;
        debugLog(
          `Failed with format "${formattedRole}": ${formatError.message}`
        );
      }
    }
    ...
    throw (
      lastError ||
      new Error(`Không thể tìm thấy người dùng với vai trò "${role}"`)
    );
  } catch (error) {
...
// API: Tìm kiếm người dùng theo tên với fallback
export const searchUserByName = async (name) => {
  try {
    debugLog(`Searching users by name: ${name}`);

    if (!name || !name.trim()) {
      throw new Error("Tên tìm kiếm không được để trống");
    }

    // For now, skip API search entirely to avoid console spam
    // and always use client-side fallback until backend is fixed
    debugLog(
      `Skipping API search due to backend issues, using client-side search for: "${name}"`
    );
    throw new Error(`API_SEARCH_FAILED`);

    /* TEMPORARILY DISABLED API SEARCH TO AVOID CONSOLE ERRORS
    const trimmedName = name.trim();
    
    // Try different API endpoint formats that might work
    const searchEndpoints = [
      `/api/User/Search-User-By-Name?name=${encodeURIComponent(trimmedName)}`,
      `/api/User/SearchUserByName?name=${encodeURIComponent(trimmedName)}`,
      `/api/User/search?name=${encodeURIComponent(trimmedName)}`,
      `/api/User/Search?name=${encodeURIComponent(trimmedName)}`
    ];
    ...
    for (const endpoint of searchEndpoints) {
      try {
        ...
        // Only log successful attempts
        if (response.ok) {
          debugLog(`Search API call successful: ${API_BASE_URL}${endpoint}`);
          const data = await handleResponse(response);
          debugLog(`Search by name "${name}" completed successfully with endpoint: ${endpoint}`, data);
          return data;
        } else if (response.status === 404 || response.status === 400) {
          // These are expected errors, silently continue to next endpoint
          continue;
        } else {
          // For other unexpected errors, log them but continue
          debugLog(`Unexpected response status ${response.status} for endpoint: ${endpoint}`);
          continue;
        }
      } catch (endpointError) {
        // Handle AbortError, network errors, etc. silently for expected cases
        if (endpointError.name === 'AbortError') {
          debugLog(`Request timeout for endpoint: ${endpoint}`);
        } else if (!endpointError.message.includes('404') && 
                   !endpointError.message.includes('400') && 
                   !endpointError.message.includes('ERR_ABORTED') &&
                   !endpointError.message.includes('Failed to fetch')) {
          debugLog(`Network error with endpoint "${endpoint}": ${endpointError.message}`);
        }
    ...
    // If all endpoints failed, throw a clean error for fallback
    debugLog(`All search API endpoints failed for "${name}", falling back to client-side search`);
    throw new Error(`API_SEARCH_FAILED`);
    */
  } catch (error) {
    // Only log significant errors, not expected API failures
    if (error.message !== "API_SEARCH_FAILED") {
      debugLog(`Unexpected error searching users by name "${name}"`, error);
