...
class MockHealthCheckApi {
  ...
  async getHealthCheckById(id) {
    await new Promise((resolve) => setTimeout(resolve, 500));
    const healthCheck = this.mockHealthChecks.find(
      (h) => h.healthCheckID === id
    );
    if (!healthCheck) {
  ...
  async updateHealthCheck(id, healthCheckData) {
    await new Promise((resolve) => setTimeout(resolve, 800));
    const index = this.mockHealthChecks.findIndex(
      (h) => h.healthCheckID === id
    );
    if (index === -1) {
  ...
  async deleteHealthCheck(id) {
    ...
    if (index === -1) {
      throw new Error("Không tìm thấy hồ sơ sức khỏe để xóa");
    }
